
import Hero3 from "@/components/ui/hero/Hero3";
import Footer from "@/components/ui/Footer";
import ContactForm from "@/components/ui/ContactForm";
import PageSection from "@/components/ui/PageSection";
import { Metadata } from "next";
import { getTranslations } from 'next-intl/server';
import { useTranslations } from 'next-intl';
import { buildLocalizedMetadata, resolveLocaleParam } from '@/lib/seo';

export async function generateMetadata({
     params,
}: {
     params: { locale?: string } | Promise<{ locale?: string }>;
}): Promise<Metadata> {
     const locale = await resolveLocaleParam(params);
     const t = await getTranslations({ locale, namespace: 'frontend.adaptedTransport.meta' });

     return buildLocalizedMetadata({
          locale,
          path: '/adapted-stay/adapted-transport',
          title: t('title'),
          description: t('description'),
     });
}

export default function AdaptedTransport() {
     const t = useTranslations('frontend.adaptedTransport');

     return (
          <>
               <Hero3
                    title={t('hero.title')}
                    description={t('hero.description')}
               />
               <main className="pt-40">
                    <PageSection
                         imageSrcDesktop="/tr.jpeg"
                         imageSrcMobile="/tr.jpeg"
                         imageAlt={t('content.title')}
                         title={t('content.title')}
                         paragraphs={[
                              t('content.paragraph1'),
                              t('content.paragraph2'),
                              t('content.paragraph3'),
                         ]}
                    />
                    <div className="mx-5 md:mx-auto max-w-7xl text-[1.3rem] mb-20 mt-5">
                         <ul className="md:px-14">
                              <li>
                                   {t('content.journeys.title')}
                                   <ol className="ps-5 mt-2 space-y-1 list-disc list-inside">
                                        <li>{t('content.journeys.item1')}</li>
                                        <li>{t('content.journeys.item2')}</li>
                                        <li>{t('content.journeys.item3')}</li>
                                   </ol>
                              </li>
                              <li className="mt-5">
                                   {t('content.drivers.title')}
                                   <ol className="ps-5 mt-2 space-y-1 list-inside">
                                        <li>{t('content.drivers.description')}</li>
                                   </ol>
                              </li>
                         </ul>
                    </div>
                    {/* <PriceCard
                         price="1000"
                         options={[
                              "Option numerscvdsdvsdvsdvsdvsdvsdvsavsdvo un",
                              "Option numerscvdsdvsdvsdvsdvsdvsdvsavsdvo un",
                              "Option numerscvdsdvsdvsdvsdvsdvsdvsavsdvo un",
                              "Option numerscvdsdvsdvsdvsdvsdvsdvsavsdvo un",
                              "Option numerscvdsdvsdvsdvsdvsdvsdvsavsdvo un",
                              "Option numerscvdsdvsdvsdvsdvsdvsdvsavsdvo un",
                         ]}
                    /> */}
                    <ContactForm />
               </main>
               <Footer />
          </>
     );
}
