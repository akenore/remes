...
export default function PriceCard({ price, options }: CardProps) {
     return (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 bg-linear-to-l from-dark-blue to-blue md:max-w-7xl mx-5 md:mx-auto px-10 py-20 mb-52 shadow-2xl">
                    ...
                    <ul className="w-full space-y-2 text-white">
                         { (Array.isArray(options) ? options : [options]).map((option, idx) => (
                              <li className="flex items-start" key={idx}>
                                   ...
                                   <span className="flex-1 wrap-break-word min-w-0 text-[1rem] md:text-[1.3rem]">{option}</span>
                              </li>
                         )) }
                    </ul>
               </div>
